...
**Порядок объявления имеет значение.** FastRoute запрещает «затенение»: если статический/менее жадный роут объявлен ПОСЛЕ уже объявленного переменного (с плейсхолдером), который его покрывает, на этапе `dispatch()` будет выброшено `BadRouteException` (пример: `Static route "/makeQR/" is shadowed by previously defined variable route "/{url}"`). Поэтому декларируйте роуты от менее жадных к более жадным и всегда ставьте «поглощающий» переменный роут вида `/{url}[/]` последним.
